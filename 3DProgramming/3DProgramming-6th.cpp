...
void Update()
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        this_thread::sleep_for(chrono::milliseconds((int)(1.0 / renderDuration.count())));

        transform.translate = glm::mat3(
            1, 0, 0,
            0, 1, 0,
            translateMat += 0.001f, 0, 1
        );

        transform.rotation = glm::mat3(
            glm::cos(glm::radians(rotationMat += 1)), -glm::sin(glm::radians(rotationMat += 1)), 0,
            glm::sin(glm::radians(rotationMat += 1)), glm::cos(glm::radians(rotationMat += 1)), 0,
            0, 0, 1
        );
        transform.scale = glm::mat3(
            scaleMat, 0, 0,
        ...
        scaleMat = scaleMat + scalePlus;
        if (scaleMat >= 1.3)
        {
            scalePlus = -0.01;
        }
        else if (scaleMat <= 0.7)
        {
            scalePlus = 0.01;
        }
        //////////////////////////////////////////////////////////////////////////////////////////
