...
public:
    ...
    void Identity()   //단위 행렬
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            Mat[i][i] = 1;    // 주대각에 1집어넣기
        }
    }

    void Transpose()
    {
        float transMat[3][3] = {0}; //전치할 별도 행렬 생성

        for (int i = 0; i < 3; i++){ //전치
            for (int j = 0; j < 3; j++)
            {
                transMat[j][i] = Mat[i][j]; //전치의 형태는 [x][y] = [y][x] 형태인 방법을 사용
            }
        }
        
        for (int i = 0; i < 3; i++) {  //기존 행렬에 전치 행렬 값 대입
            for (int j = 0; j < 3; j++)
            {
